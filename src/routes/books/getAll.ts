...
function filterBook(req: Hapi.Request, allBooks: AllBooks) {
  return books.map((book) => {
    ...
    if (req.query.reading) {
      ...
      if (req.query.reading === "1" && book.reading) {
        return allBooks.push({
          id: book.id as string,
          name: book.name,
          publisher: book.publisher,
      ...
      if (req.query.finished === "1" && book.finished) {
        return allBooks.push({
          id: book.id as string,
          name: book.name,
          publisher: book.publisher,
...
const getAllBooks: Hapi.ServerRoute = {
  method: "Get",
  path: "/books",
